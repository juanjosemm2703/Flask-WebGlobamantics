...
from flask import Flask, jsonify, flash, g, redirect, render_template, request, url_for, send_from_directory
from flask_wtf import FlaskForm, RecaptchaField
from flask_wtf.file import FileAllowed, FileRequired
from wtforms import StringField, DecimalField, TextAreaField, SubmitField, SelectField, HiddenField, FileField
...
app.config["IMAGE_UPLOADS"] = os.path.join(basedir, "upload")
# ReCAPTCHA configuracion con Google
app.config["RECAPTCHA_PUBLIC_KEY"] = "6LeYnaUcAAAAAMM7kxVzq9C29cEWlt5FbsSpTz3H"
app.config["RECAPTCHA_PRIVATE_KEY"] = "6LeYnaUcAAAAAOqEsGXX7HMi3R0X-mZky6JUcsqE"
# configurar la app como testing
...
class CustomSelectField(SelectField):
    def __init__(self, label=None, validators=None, coerce=int, choices=None, table=None, columns=[], allow_blank=False,
                 **kwargs):
    ...
    def pre_validate(self, form):
        ...
        for v, _ in rows:
            ...
            raise AttributeError("The chosen option does not exist")

    def get_rows(self):
        c = get_db().cursor()
        try:
            c.execute(f"SELECT {self.columns[0]}, {self.columns[1]} FROM {self.table}")
        except:
...
class BelongsToOtherFieldOption:
    def __init__(self, table, belongs_to, foreign_key=None, message=None):
        if not table:
            ...
            BelongsToOtherFieldOption validator needs the table
            """)
        if not belongs_to:
            raise AttributeError("""
    ...
    def __call__(self, form, field):
        ...
        try:
            c.execute("""SELECT COUNT(*) FROM {}
                        WHERE id = ? AND {} = ? """.format(self.table, self.foreign_key),
...
# Nuevo validador, evitar que envien una subcategoria que no pertenece a la categoria
...
class NewItemForm(ItemForm):
    category = CustomSelectField("Category", coerce=int, table="categories", columns=["id", "name"])
    subcategory = CustomSelectField("Subcategory", coerce=int, table="subcategories", columns=["id", "name"],
                                    validators=[
...
def home():
    ...
    # Verifica si tiene un parametro AJAX
    try:
...
def edit_item(item_id):
    ...
    if item:
        ...
        if form.validate_on_submit():
            ...
            c.execute("""UPDATE items SET title = ?, description = ?, price = ?, image = ?
                        WHERE id =?""",
                      (
...
def item(item_id):
    ...
    if item1:
        comments_from_db = c.execute("""SELECT content FROM comments WHERE item_id = ?
        ...
        return render_template("item.html", item=item1, comments=comments, commentForm=commentForm,
                               deleteItemForm=deleteItemForm)
    return redirect(url_for("home"))
